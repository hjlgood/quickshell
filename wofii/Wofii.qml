...
PanelWindow {
    ...
    Rectangle {
        ...
        MouseArea { anchors.fill: parent; onClicked: {} } // Block click-through to background
        ...
        ColumnLayout {
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            // Launch app
                            runner.command = ["sh", "-c", model.exec + " &"] // Detach
                            runner.running = true
                            rootWofii.visible = false
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                           ...
                           Text { 
                               anchors.centerIn: parent; 
                               text: (model.name || "?")[0]; 
                               color: Colors.backgroundDark; font.bold: true 
                           }
                        }
            ...
            Process {
                ...
                command: ["sh", "-c", "grep -E '^(Name|Exec)=' -r /usr/share/applications ~/.local/share/applications -h | awk -F= '{if($1==\"Name\"){n=$2}else if($1==\"Exec\"){print n \"|\" $2}}'"]
                    ...
                    onStreamFinished: {
                        ...
                        for (var i = 0; i < lines.length; i++) {
                            ...
                            if (parts.length >= 2) {
                                ...
                                tempApps.push({name: name, exec: exec})
                            }
                        }
                        
                        // Sort content
                        tempApps.sort(function(a, b) { return a.name.localeCompare(b.name); })
            ...
            Connections {
                target: searchInput
                function onTextChanged() { rootWofii.updateList(searchInput.text) }
            }
        }
    }
